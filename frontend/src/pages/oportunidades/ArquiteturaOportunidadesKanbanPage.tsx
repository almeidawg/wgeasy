...
export default function OportunidadesKanbanPage() {
  ...
  async function carregarOportunidades() {
    ...
    if (clienteIds.length > 0) {
      ...
      if (clientesData) {
        clientesMap = clientesData.reduce(
          (acc: Record<string, OportunidadeClienteUI>, cli: any) => {
            acc[cli.id] = {
              id: cli.id,
              nome: cli.nome,
              email: cli.email,
    ...
    const oportunidadesComDados = (oportData || []).map((op: any) => ({
      ...
      nucleos:
        nucleosData?.filter((n: any) => n.oportunidade_id === op.id) || [],
    }));
  ...
  async function criarContratosAutomaticamente(oportunidade: Oportunidade) {
    try {
      ...
      if (nucleos.length === 0) {
        console.log(
          "Oportunidade sem núcleos definidos, não será criado contrato"
        );
        return;
      }
      ...
      for (const nucleo of nucleos) {
        ...
        if (contratoError) {
          console.error(
            `Erro ao criar contrato para ${nucleo.nucleo}:`,
            contratoError
          );
        } else {
          console.log(
