...
// Upload, visualização, download e exclusão via Supabase Storage
// Integração com Google Drive para backup e compartilhamento
// ============================================================
...
export default function OportunidadeArquivos({ oportunidadeId, clienteNome = 'Cliente' }: OportunidadeArquivosProps) {
  ...
  // Estados para seleção de pasta no upload
  ...
  /**
   * Verifica se já existe sessão ativa do Google Drive
   */
  async function checkDriveAuth() {
    if (googleDriveService.isAuthenticated()) {
      setDriveConnected(true);
      console.log('✅ Google Drive já autenticado (sessão existente)');
      ...
      try {
        ...
        if (pastaExiste) {
          ...
          // Carregar todas as subpastas para seleção
  ...
  /**
   * Abre o dialog de seleção de pasta antes de fazer upload
   */
      ...
      {showFolderDialog && pendingFiles && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] overflow-hidden">
            ...
            {/* Seleção de pasta local (Supabase) */}
            ...
            {/* Seleção de pasta do Drive (se conectado) */}
