...
# -*- coding: utf-8 -*-
"""
Upload robusto com reconexao automatica
"""
...
def upload_com_retry(local_file, remote_file, max_tentativas=3):
    """Upload com reconexao automatica"""
...
def criar_dir_com_retry(remote_dir):
    """Criar diretorio com reconexao"""
    try:
        ftp = conectar()
        ftp.mkd(remote_dir)
        ftp.quit()
...
def main():
    print("=" * 60)
    print("  UPLOAD ROBUSTO - Site WG Almeida")
    print("  Destino: /public_html")
    print("=" * 60)
    print()
    ...
    print(f"Total de arquivos para enviar: {len(arquivos)}")
    print(f"Total de diretorios: {len(diretorios)}")
    print()

    # Criar diretorios
    print("Criando diretorios...")
    for d in sorted(diretorios):
        criar_dir_com_retry(d)
